...
{
    public partial class BeslenmeProgrami : KryptonForm
    {
        public BeslenmeProgrami()
        {
            InitializeComponent();
        }
        SQLBaglanti baglan = new SQLBaglanti();
        public string beslenmeid;


        private void BeslenmeProgrami_Load(object sender, EventArgs e)
        {
            Lbl_Beslenme_ID.Text = beslenmeid;

            SqlCommand bsl = new SqlCommand("Select * from UyeBilgi where UyeID=@p1",baglan.baglanti());
            bsl.Parameters.AddWithValue("@p1", Lbl_Beslenme_ID.Text);
            SqlDataReader dr = bsl.ExecuteReader();
            while (dr.Read())
            {
        ...
        private void Btn_Program_Temizle_Click(object sender, EventArgs e)
        {
            Rch_Program.Clear();
        }

        private void Btn_Program_Kaydet_Click(object sender, EventArgs e)
        {
            SqlCommand bslkaydet = new SqlCommand("Update UyeBilgi set UyeBeslenmeProgrami=@p1 where UyeID=@p2", baglan.baglanti());
            bslkaydet.Parameters.AddWithValue("@p1", Rch_Program.Text);
            bslkaydet.Parameters.AddWithValue("@p2", Lbl_Beslenme_ID.Text);
            bslkaydet.ExecuteNonQuery();
